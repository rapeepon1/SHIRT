...
const Order = () => {
  ...
  return (
    <div className="order-page">
        ...
        {orders.map((order) => (
          <div key={order.order_id} className="order-card" onClick={() => setSelectedOrder(order)}>
            ...
            <div className="order-summary-info">
              <p>วันที่: {new Date(order.order_date).toLocaleString('th-TH')}</p>
      ...
      {/* Modal แสดงรายละเอียดสินค้าที่สั่ง */}
      {selectedOrder && (
        <div className="modal-overlay" onClick={() => setSelectedOrder(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <p><strong>ผู้สั่งซื้อ:</strong> {selectedOrder.ship_name}</p>
              <p><strong>เบอร์โทรศัพท์:</strong> {selectedOrder.ship_phone}</p>
              <p><strong>ที่อยู่จัดส่ง:</strong> {selectedOrder.ship_address}</p>
              <p><strong>วันที่สั่งซื้อ:</strong> {new Date(selectedOrder.order_date).toLocaleString('th-TH')}</p>
              ...
              <div className="order-modal-total">
                ...
                <span>{Number(selectedOrder.total_amount).toLocaleString()} THB</span>
              </div>
            </div>
            <button className="btn-modal-close" onClick={() => setSelectedOrder(null)}>ปิดหน้าต่าง</button>
          </div>
        </div>
      )}
    </div>
